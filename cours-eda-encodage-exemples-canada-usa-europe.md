...
|  55 | USA     | Basic    | faible       |              7 | Oui   |
|  28 | Allemagne | Premium  | élevée       |              0 | Non   |
...
Si le modèle n’a jamais vu `Allemagne`, l’encodeur peut générer une erreur.
...
```text
ville = Montréal, Toronto, Vancouver, Ottawa, Calgary, Québec, ...
```
...
df = pd.DataFrame({
    ...
    "pays": ["Canada", "France", "Canada", "USA", np.nan, "Allemagne"],
...
```text
Le modèle peut croire que USA > France > Canada.
```
